...
import { Permission, Role11 } from "../../utils/types";

interface RoleModalProps {
  role: Role11 | null;
  availablePermissions: Permission[];
  onClose: () => void;
  onSave: (role: Role11) => void;
}
...
const RoleModal: React.FC<RoleModalProps> = ({
  ...
  const [formData, setFormData] = useState<Role11>(
  ...
  const handlePermissionToggle = (permissionId: string) => {
    setFormData((prev: any) => ({
      ...
      permissions: prev.permissions.includes(permissionId)
        ? prev.permissions.filter((id: any) => id !== permissionId)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      try {
        if (!formData.name.trim()) throw new Error("Role name is required.");
        onSave(formData);
        setLoading(false);
      } catch (err) {
